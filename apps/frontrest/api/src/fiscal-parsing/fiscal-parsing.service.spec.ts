...
describe('FiscalParsingService', () => {
  ...
  describe('parse — documento completo e bem formado', () => {
    const result = service.parse(REALISTIC_INVOICE);
  ...
  describe('parse — texto sem nenhuma informação fiscal reconhecível', () => {
    it('só o fornecedor é encontrado (fallback de 1ª linha, confiança baixa) — todos os outros campos null', () => {
      const result = service.parse('texto sem qualquer informação fiscal reconhecível');
    ...
    it('devolve absolutamente todos os campos null e confiança 0 para texto vazio (nem o fallback do fornecedor encontra uma 1ª linha)', () => {
  ...
  describe('parse — documento parcial (só alguns campos presentes)', () => {
    it('devolve null apenas para os campos não encontrados, preservando os restantes', () => {
      const result = service.parse('Fatura N.º: FA2026/1\nTotal a Pagar: 100,00€');
  ...
  describe('independência dos extractors', () => {
    it('cada extractor corre sobre o mesmo texto de forma isolada — um extractor lançar não é possível aqui, mas um "null" de um não afeta os outros', () => {
      const result = service.parse('Cliente: Restaurante Sabor Único, Lda');
  ...
  describe('vários extractors a competir pelo mesmo campo (ex. um extractor por país no futuro)', () => {
    it('vence o de maior confiança, independentemente da ordem de registo', () => {
      ...
      const weakFirst = new FiscalParsingService([weak, strong]).parse('qualquer texto');
      const strongFirst = new FiscalParsingService([strong, weak]).parse('qualquer texto');
    ...
    it('em empate exato de confiança, vence o primeiro extractor a ser registado (regra determinística, não arbitrária)', () => {
      ...
      const result = new FiscalParsingService([first, second]).parse('qualquer texto');

      expect(result.supplierTaxId?.value).toBe('PRIMEIRO');
    });

    it('metadata.fieldsFound não duplica o campo mesmo com dois extractors a alimentá-lo', () => {
      ...
      const result = new FiscalParsingService([a, b]).parse('qualquer texto');
