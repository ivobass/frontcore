...
| Fase 6.11 — AI Provider Foundation | `docs/phases/phase-6.11-ai-provider-foundation.md` | Registo da Fase 6.11 — `@frontcore/ai` transformado em package operacional: contrato normalizado, `loadAiConfig()`, `createAiProvider()`, `MockAiProvider`, `OllamaAiProvider` (primeiro provider real, local, API nativa do Ollama via `fetch`, sem SDK, sem API key, timeout via `AbortController`, erros sanitizados via `AiProviderError`); zero consumidor real ainda, nenhum extractor de IA implementado; OpenAI/Anthropic ficam para fase futura | Fases | Concluído | `docs/PHASES.md`; `docs/ARCHITECTURE.md`; `docs/adr/0007-document-extraction-foundation.md`; commit/tag por criar |
