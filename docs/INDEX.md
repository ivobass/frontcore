...
| ADR-0008 | `docs/adr/0008-financial-analysis-engine-foundation.md` | Separação entre Financial Insights (factos) e Financial Analysis Engine (conclusões determinísticas), módulo único `financial-analysis/`, sem novo package, exclusão de `ai/` | ADRs | Aceite | `docs/phases/phase-8.10-financial-analysis-engine-foundation.md` |
...
| Fase 6.14 — AI Invoice Extraction & Draft Items Foundation | `docs/phases/phase-6.14-ai-invoice-extraction-draft-items-foundation.md` | Registo da Fase 6.14 — `@frontcore/ai` ganha structured output aditivo (`responseFormat`: OpenRouter traduzido e confirmado real, Mock determinístico, Ollama erro controlado `unsupported_capability`); `AiInvoiceExtractionV1`/`AiInvoiceExtractor` (`apps/frontrest/api/src/ai-invoice-extraction/`, segundo consumidor de `AI_COMPLETION_PROVIDER`) complementam o parsing fiscal determinístico (Fase 6.6) sem o substituir; `InvoiceExtractionMerger` reconcilia por campo, nunca escolhe automaticamente num conflito; novo `InvoiceDraftItem`/`InvoiceDraftAiExtraction` (migration `20260816181603`, contém `ALTER COLUMN`, nunca "puramente aditiva"); `InvoiceDraft.itemsReviewedByHuman` protege correções humanas; `InvoiceItem` evoluído (`position`/`unit`/`vatRate`, `Decimal(11,3)`/`Decimal(14,4)` preservando a magnitude inteira anterior); `POST :id/ai-extraction`, `PUT :id/items`, `PATCH :id/review` (cabeçalho+linhas atómicos), `promote()` a copiar linhas sob lock `SELECT ... FOR UPDATE` (bloqueia com linha incompleta, nunca perde alteração concorrente); UI de revisão ganha secção de linhas + "Analisar com IA"; validado com 5 documentos reais contra OpenRouter real (`google/gemini-2.5-flash`); regressão da Fase 6.13 intacta; sem multimodal/promoção automática/inventário/billing; correções pós-revisão Codex em 2 rondas: 1ª ronda — parser estrutural estrito, Decimal corrigido, atomicidade items/metadata, `PATCH :id/review` atómico, multi-tenant; 2ª ronda — política única de locking `SELECT ... FOR UPDATE` (`lockInvoiceDraftRow()`) partilhada por `runAiExtraction()`/`replaceItems()`/`saveReview()`/`promote()` (a 1ª correção da corrida IA vs. humano era insuficiente — só uma releitura sem lock), provada contra Postgres real (`test:integration`), política de negativos de linha alinhada com `InvoiceDraftItemDto`, `position` a exigir a ordem exata do array | Fases | Concluído | `docs/PHASES.md`; `docs/ARCHITECTURE.md`; `docs/phases/phase-6.13-document-regression-test-suite.md`; `docs/phases/phase-6.11-ai-provider-foundation.md`; commit/tag por criar |
...
| Fase 8.8 — Financial AI Reliability & Strict Grounding Foundation | `docs/phases/phase-8.8-financial-ai-reliability-strict-grounding-foundation.md` | Registo da Fase 8.8 — cinco reforços de fiabilidade sobre a arquitetura já existente (Fases 8.1–8.7), sem alterar nenhuma estruturalmente: Strict Grounding a nível de prompt (`ASSISTANT_RULES`/`TOOL_ATTEMPT_RULES` proíbem alterar/reformular dados já fornecidos; resposta só espaço em branco tratada como inconsistente); Router Hardening (`saldo`/`extrato`/`preço`/`cobrança`, nunca a forma nua "cobra"); Financial Conversation Context Hardening (`parseFinancialConversationContext()` valida calendário real e `recordedAt` parseável, nunca filtro vazio, `try`/`catch` total — corrige um bug real de crash); Prompt Injection Hardening (`sanitizeDomainText()` em nomes de fornecedor/categoria); **Strict Grounding estrutural (correção obrigatória)** — novo `financial-grounding.validator.ts` (`validateFinancialGrounding()`), fronteira determinística entre `FinancialRetrievalResult` e a resposta final, nunca o LLM como validador: rejeita qualquer valor/contagem/data na resposta que não pertença ao conjunto real extraído do resultado tipado, exige presença do fornecedor/categoria/estado real quando `filters` o define; resposta rejeitada nunca persistida, substituída por `buildFinancialContextMessage(result)` marcada `financial-grounding-fallback`; aplicada ao caminho direto e à resposta final após tool calling; sem OCR, Fiscal Parsing, InvoiceDraft, promoção, IVA, NIF, Dashboard, Reports, tools novas, providers novos, embeddings, RAG, agentes, streaming, package novo, migration, alteração Prisma, frontend ou tool registry. Hardening pós-validação manual 2026-08-15 (4 problemas reais corrigidos): `DashboardService.byStatus` (Fase 7) passa a respeitar `query.status` explícito, nunca mais combina o universo filtrado com `outstanding` não filtrado; continuação elíptica ("E as pagas?") substitui o filtro de estado herdado; "falta pagar" reconhecida (`OUTSTANDING_BALANCE`); `validateFinancialGrounding()` ganha uma 5ª categoria (`invoiceNumbers`, rótulo explícito "número"/"número da fatura") — ver secção "Hardening pós-validação manual" no documento da fase | Fases | Concluído | `docs/PHASES.md`; `docs/phases/phase-8.7-financial-conversation-context-foundation.md`; `docs/phases/phase-7-financial-dashboard-foundation.md`; commit/tag por criar |
| Fase 8.9 — Financial Insights Foundation | `docs/phases/phase-8.9-financial-insights-foundation.md` | Registo da Fase 8.9 — camada determinística de Financial Insights (concentração, ranking, saldo por pagar, maior fatura/fornecedor/categoria, tendência), novo módulo puro `apps/frontrest/api/src/financial-insights/` (`buildFinancialInsights()` e helpers, nunca um serviço), calculada a partir de `DashboardService.getFinancialSummary()`/`getLargestInvoices()` (inalterados) e reutilizada sem duplicação por Chat IA (`FINANCIAL_SUMMARY` ganha `insights`), novo `GET /dashboard/financial-insights`, e `ReportsService`/`MonthlyFinancialReport` (JSON/CSV/PDF); contrato `FinancialInsights` separado de `FinancialDashboardSummary` (`SupplierInsight`/`CategoryInsight`/`OutstandingInsight`/`LargestExpenseInsight`/`TrendInsight`); `share`/percentagens sempre string decimal a 2 casas via `Prisma.Decimal`, nunca `number`; `FINANCIAL_INSIGHTS_SUPPLIER_TOP_N`/`CATEGORY_TOP_N = 3` fixos; chamadas independentes sempre em `Promise.all`; Strict Grounding (Fase 8.8) estendido com nova categoria `percentages` (normalizada, sem tolerâncias), nunca enfraquecido; frontend `/dashboard`/`/reports` ganham secção "Destaques"; sem filtros HTTP novos, relatório anual, intervalo personalizado, forecasting, recomendações, scoring, agentes, `InvoiceItem`, package novo ou migration | Fases | Concluído | `docs/PHASES.md`; `docs/ARCHITECTURE.md`; `docs/phases/phase-8.8-financial-ai-reliability-strict-grounding-foundation.md`; commit/tag por criar |
| Fase 8.10 — Financial Analysis Engine Foundation | `docs/phases/phase-8.10-financial-analysis-engine-foundation.md` | Registo da Fase 8.10 — motor determinístico de análise financeira, novo módulo único `apps/frontrest/api/src/financial-analysis/` (irmão de topo de `financial-insights/`, sem package novo, sem colocação em `ai/`); três níveis explícitos (Financial Insights → Financial Analysis Engine → consumidores, nenhum integrado ainda); contrato `FinancialAnalysis<TId,TConclusion,TEvidence>` síncrono, união discriminada fechada `FinancialAnalysisOutcome`, motor `runFinancialAnalyses()` sem `processingTimeMs`; duas análises demonstradoras (`monthly_trend`, `relative_concentration`, sem limiar/scoring); sem aging, rankings completos, forecasting, ML, agentes, alteração a `financial-insights/`, schema Prisma ou integração em Chat/Dashboard/Reports | Fases | Concluído | `docs/PHASES.md`; `docs/ARCHITECTURE.md`; `docs/adr/0008-financial-analysis-engine-foundation.md`; `docs/phases/phase-8.9-financial-insights-foundation.md`; commit `3dcd6bc` |
| Fase 8.11 — Dashboard Financial Analysis Integration Foundation | `docs/phases/phase-8.11-dashboard-financial-analysis-integration-foundation.md` | Registo da Fase 8.11 — Dashboard como primeiro consumidor real do Financial Analysis Engine (Fase 8.10); novo `GET /dashboard/financial-analysis` (`{ insights, analysis }`), `DashboardController` fino (delega, `organizationId` só de `CurrentUser`), composição integral (paralelismo, construção de `FinancialInsights`, seleção explícita das duas análises, execução do motor) num novo método do `DashboardService` existente, sem serviço NestJS novo; frontend `/dashboard` substitui `getFinancialInsights()` por `getDashboardFinancialAnalysis()` (dois pedidos, nunca três) e ganha a secção "Análise financeira", sem recálculo nem inferência; sem novas análises/métricas/thresholds/scoring/forecasting/persistência/cache/schema Prisma/queries novas/integração em Reports ou AI Chat | Fases | Concluído | `docs/PHASES.md`; `docs/ARCHITECTURE.md`; `docs/phases/phase-8.10-financial-analysis-engine-foundation.md`; commit `375ed88` |
| Fase 8.12 — Reports Financial Analysis Integration Foundation | `docs/phases/phase-8.12-reports-financial-analysis-integration-foundation.md` | Registo da Fase 8.12 — Reports como segundo consumidor real do Financial Analysis Engine (Dashboard, Fase 8.11, continua o primeiro); `MonthlyFinancialReport` ganha `analysis: FinancialAnalysisEngineOutput` sempre presente (nunca opcional, ao contrário de `insights`); `ReportsService.getMonthlyReport()` constrói `FinancialInsights` uma vez e executa `runFinancialAnalyses()` com seleção própria deste módulo (`REPORTS_FINANCIAL_ANALYSES`, nunca partilhada com a do Dashboard); JSON/CSV/PDF ganham a secção "Análise financeira" (mensagem explícita quando vazia, nunca omissão silenciosa); frontend `/reports` ganha componente próprio, sem reutilizar o do Dashboard (decisão explícita); sem novas análises/métricas/thresholds/scoring/forecasting/queries Prisma/migration/persistência/cache/serviço NestJS/package/abstração partilhada/alteração ao Dashboard ou AI Chat | Fases | Concluído | `docs/PHASES.md`; `docs/ARCHITECTURE.md`; `docs/phases/phase-8.11-dashboard-financial-analysis-integration-foundation.md`; commit/tag por criar |
| Fase 8.13 — Grounded AI Financial Analysis Integration | `docs/phases/phase-8.13-grounded-ai-financial-analysis-integration.md` | Registo da Fase 8.13 — AI Chat como terceiro consumidor real do Financial Analysis Engine (Dashboard e Reports já integrados); `FinancialIntentData`'s `FINANCIAL_SUMMARY` ganha `analysis: FinancialAnalysisEngineOutput` sempre presente; `FinancialRetrievalService.resolveDataForPeriod()` (partilhado por retrieval direto e tool calling) constrói `FinancialInsights` uma vez e executa `runFinancialAnalyses()` com seleção própria (`AI_CHAT_FINANCIAL_ANALYSES`); `buildFinancialContextMessage()` ganha `buildAnalysisLines()`; `validateFinancialGrounding()` sem alteração funcional (`collectInsightFacts()` já cobre a evidência da análise, `collectAnalysisFacts()` avaliada e não criada por YAGNI); `AiToolOrchestratorService` inalterado; sem novas análises/métricas/thresholds/scoring/forecasting/agentes/RAG/embeddings/tools/intenções/queries Prisma/alteração a Dashboard/Reports/frontend | Fases | Concluído | `docs/PHASES.md`; `docs/ARCHITECTURE.md`; `docs/phases/phase-8.12-reports-financial-analysis-integration-foundation.md`; commit/tag por criar |
| Fase 10.1 — DevOps CI Foundation | `docs/phases/phase-10.1-devops-ci-foundation.md` | Registo da Fase 10.1 — primeira infraestrutura DevOps: `.github/workflows/ci.yml` (12 passos: checkout, setup pnpm/Node 20 com cache, install, Documentation Validation, Lint, build do `@frontcore/database`, typecheck, test, build, `docker compose config`, `docker compose build`; nunca `docker compose up`), executado em `push`/`pull request` para `main`; novo `scripts/validate-docs.mjs` (agregador) + 5 validadores de responsabilidade única em `scripts/validators/` (existência de `docs/INDEX.md`/`docs/PHASES.md`/`docs/ARCHITECTURE.md`/`docs/ai/`, documentos de fase referenciados por `docs/PHASES.md`, referências locais quebradas em toda a documentação); novo script `docs:validate` em `package.json`, nenhum script existente alterado; `pnpm lint` continua um no-op (nenhum package define `lint` ainda, fora do âmbito); sem deploy automático, Kubernetes, Dependabot, security scanning, code coverage, release automation, Docker Hub, VPS, backups, monitorização, alertas, múltiplos workflows ou CI matrix | Fases | Concluído (execução real no GitHub Actions pendente do primeiro push) | `docs/PHASES.md`; `docs/ARCHITECTURE.md`; commit/tag por criar |
| Fase 10.2 — DevOps CI Verification & Hardening | `docs/phases/phase-10.2-devops-ci-verification-hardening.md` | Registo da Fase 10.2 — verificação e reforço do pipeline da Fase 10.1: `workflow_dispatch`, `concurrency` (cancela execuções antigas da mesma branch/PR) e `timeout-minutes: 30` adicionados; job renomeado `quality` (`name: Quality, Tests and Build`); novo passo "API E2E Tests" (`pnpm --filter @frontrest/api test:e2e`, mockado, sem serviços Postgres/Redis/MinIO no CI) entre "Test" e "Build"; `pnpm lint` deixou de ser um no-op — ESLint 9 real (`eslint.config.mjs`, flat config, `typescript-eslint` recommended não type-aware, `@next/eslint-plugin-next` como dependência direta fixada a `15.1.3` — nunca só transitiva via `eslint-config-next`, removido por não ser usado — + `eslint-plugin-react-hooks`, regras só em `apps/frontrest/web`, plugins registados sem `files` para o `next build` os detetar corretamente, corrigindo "The Next.js plugin was not detected"), `"lint": "eslint ."` em `apps/frontrest/{api,web,workers}` e nas 11 `packages/*`; achados reais corrigidos (imports/variáveis não usados, `no-useless-escape` em 5 extractors fiscais, confirmado sem regressão nos 268 testes de `fiscal-parsing`, 2 diretivas `eslint-disable` obsoletas); `@typescript-eslint/no-explicit-any` mantida como `warn` (5 warnings conhecidos e documentados, sem mass refactor do cliente HTTP do web); badge de CI único no `README.md`; recomendações de proteção de branch documentadas, não aplicadas (sem acesso à API/CLI do GitHub); sem deploy automático, staging, produção, VPS, Kubernetes, Dependabot, CodeQL, coverage gates, observabilidade, CI matrix ou alteração à Fase 8.9 | Fases | Concluído (execução real no GitHub Actions pendente do primeiro push) | `docs/PHASES.md`; `docs/ARCHITECTURE.md`; `docs/phases/phase-10.1-devops-ci-foundation.md`; commit/tag por criar |
